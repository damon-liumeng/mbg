...
public class MyBatisGeneratorConfigurationParser {
    ...
    protected void parseTable(Context context, Node node) {
        ...
        String enableSelectByBasicParam = attributes
                .getProperty("enableSelectByBasicParam"); //$NON-NLS-1$
        if (stringHasValue(enableSelectByBasicParam)) {
            tc.setSelectByBasicParamStatementEnabled(
                    isTrue(enableSelectByBasicParam));
        }
        ...
        String selectByBasicParam = attributes
                .getProperty("selectByBasicParam"); //$NON-NLS-1$
        if (stringHasValue(selectByBasicParam)) {
            tc.setSelectByBasicParam(selectByBasicParam);
        }
